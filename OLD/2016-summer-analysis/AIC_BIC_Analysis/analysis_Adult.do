...
* Sources of data:
* 	1.	"tuples_adult.csv": The file contains combinations of baseline 
...
local outcomesAdult		all_houseOwn binSatisFamily binSatisHealth binSatisIncome binSatisWork
						...
						p50IQ_factor p75IQ_factor live_parent LocusControl Maria MaxEdu_Uni 
						MigrTaste_cat mStatus_married_cohab optimist PA_Empl Pension reciprocity1bin 
						reciprocity2bin	reciprocity3bin reciprocity4bin Reddito_1 Reddito_3 
						Reddito_4 Reddito_5 SES_self SickDays Smoke sport votoMaturita 
						votoUni WageMonth;
...
/* The following vars are exluded from list of outcomes because they either had 0 obs or there was no variance
   in the variable for at least one adult cohort.
						
   Reddito_2 Reddito_6 Reddito_7 MaxEdu_Grad i_RiskDUI i_RiskFight RiskSuspended SDQ_score 
*/
	

/*Tuples contain all combinations of 5 of the following variables:
----------------------------------------------------------------
"Male", "CAPI", "numSiblings" 
"momMaxEdu_middle", "momMaxEdu_HS", "momMaxEdu_Uni","momMaxEdu_Grad","momBornProvince",
"dadMaxEdu_low", "dadMaxEdu_middle", "dadMaxEdu_HS", "dadMaxEdu_Uni", "dadMaxEdu_Grad",
"dadBornProvince" */

cd ${klmReggio}/Analysis/AIC_BIC_Analysis

* Opening .csv file to store AIC/BIC results
file open file1 using "./Output/AIC_BIC_Adult.csv", write replace
file write file1 "Specification,Tuple,Controls,AIC,BIC,N" _n // 1st row = variable names

foreach outcome of local outcomesAdult{
	forvalues i = 1/`max_tuple'{
		local cohort_val = 4 // Adult30 = 4, Adult40 = 5, Adult50 = 6
		foreach cohort in "Adult 30" "Adult 40" "Adult 50"{
		
			// This corresponds with eq.1 and compares all Reggio adults of age `i' with Parma and Padova adults of age 'i'
			
			quietly reg	`outcome' Reggio `tuple_`i'' if Cohort == `cohort_val'
			quietly estat ic //command creates matrix r(S) where 1st, 5th and 6th elements are N, AIC and BIC respectively 
			matrix aa =  r(S)
			local num =  aa[1,1]
			local aic =  aa[1,5]
			local bic =  aa[1,6]
			file write file1 "1,tuple_`i',`tuple_`i'',`aic',`bic',`num'" _n

			
			// This corresponds with eq.3 and compares all Reggio adults of age `i' who attended materna with Parma and Padova
			// adults of age 'i' who attended materna
		
			quietly reg	`outcome' Reggio `tuple_`i'' if ((Cohort == `cohort_val')&(materna==1))
			quietly estat ic //command creates matrix r(S) where 1st, 5th and 6th elements are N, AIC and BIC respectively 
			matrix aa =  r(S)
			local num =  aa[1,1]
			local aic =  aa[1,5]
			local bic =  aa[1,6]
			file write file1 "2,tuple_`i',`tuple_`i'',`aic',`bic',`num'" _n
			
			
			// This corresponds with eq.4 and compares Reggio adults of age `i' who went to materna with Reggio adults
			// with did not attend materna

			quietly reg	`outcome' materna_Municipal materna_State materna_Religious materna_Private `tuple_`i''	if ((Cohort == `cohort_val')&(Reggio==1))
			quietly estat ic //command creates matrix r(S) where 1st, 5th and 6th elements are N, AIC and BIC respectively 
			matrix aa =  r(S)
			local num =  aa[1,1]
			local aic =  aa[1,5]
			local bic =  aa[1,6]
			file write file1 "3,tuple_`i',`tuple_`i'',`aic',`bic',`num'" _n
			
			local cohort_val = `cohort_val'+1
		}
		
		// This simple estimation with no restriction on sample identifies the tuples that best describe outcomes
		
		quietly reg	`outcome' `tuple_`i'' if Cohort > 3
		quietly estat ic //command creates matrix r(S) where 1st, 5th and 6th elements are N, AIC and BIC respectively 
		matrix aa =  r(S)
		local num =  aa[1,1]
		local aic =  aa[1,5]
		local bic =  aa[1,6]
		file write file1 "4,tuple_`i',`tuple_`i'',`aic',`bic',`num'" _n
	}
}

file close file1
*-------------------------------------------------------------------------------*
* Generating .csv files containing mean AIC/BIC scores
*-------------------------------------------------------------------------------*
* We first generate a .csv file that averages the score across all specifications

cd ${klmReggio}/Analysis/AIC_BIC_Analysis

* Importing file with AIC_BIC scores that was created above
import delimited "./Output/AIC_BIC_Adult.csv", varnames(1) delimiters(",") clear
...
file open file2 using "./Output/Average_AIC_BIC_Adult_Combined.csv", write replace
...
file close file2
*--------------------------------------
* generating .csv files that average scores by different specifications
...
forvalues j = 1/4{
	...
	* Only keeping estimations from specification `j'
	drop if specification != `j'
	...
	* Opening .csv file to store AIC/BIC average results
	file open file2 using "./Output/Average_AIC_BIC_Adult_Spec`j'.csv", write replace
	file write file2 "tuple,controls, Mean AIC, Mean BIC" _n // 1st row = variable names
