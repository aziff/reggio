...
** Controls
local Xright		  	    Male momMaxEdu_* dadMaxEdu_* internr_* CAPI momBornProvince ///
							dadBornProvince cgRelig houseOwn cgReddito_* Age Age_sq 
...
label var childSDQ_score 	"SDQ (mom rep.)"
label var SDQ_score 		"SDQ (self rep.)"
label var MigrTaste_cat		"Migration taste"
...
if 1==0{ // Graphs of summary statistics for outcomes
* ---------------------------------------------------------------------------- *
				...
				* 4. All controls
				reg `outcome' `main_terms' `Xright' `Xleft`cohort'' if `large_sample_condition', robust  
					...
					*outreg2 using "${klmReggio}/Analysis/Output/test`city'`age'`cohort'``outcome'_short'.out", append `outregOption`cohort'' addtext(Controls, all) 
					outreg2 using "${klmReggio}/Analysis/jk-az-analysis/Output/Xleft/test`city'`age'`cohort'``outcome'_short'all.tex", replace `outregOption`cohort'' addtext(Controls, all)
