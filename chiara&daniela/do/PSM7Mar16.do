...
* 3) PSM 
* 4) PSM (CLOSEST DEMAND, AND THEN CLOSER SUPPLY)
*** FOCUS ON "MATERNA"
* 5) OLS (DUMMY MATERNA)
* 6) PSM 
* 7) PSM (CLOSEST DEMAND, AND THEN CLOSER SUPPLY)
...
tab grandDist
tab grandDist, nol m
//gen grandp_close = grandDist <= 4 //this is the same as grand_city
...
probit nido score score2 dist175m grand_city $controls if(Reggio == 1)
...
biprobit (supply = score score2 $controls) (demand = dist175m grand_city lone_parent childrenSibIn $controls) if(Reggio == 1), partial difficult
...
probit ReggioMaterna low_score med_score distMaternaMunicipal1 grand_city $controls if(Reggio == 1)
...
biprobit (supply = low_score med_score $controls) (demand = distMaternaMunicipal1 $controls) if(Reggio == 1), partial difficult
