...
Although the baseline OLS and difference-in-difference models attempt to account for the selection into the Reggio Approach, regression estimator for ATE may be poorly estimated when the observed values of baseline variables are not similar for the treatment and control groups \citep{Glynn-Quinn_2010_Political-Analysis}. We estimate an Augmented Inverse Propensity Weighted (AIPW) estimator that builds on the multinomial logit presented above in order to account for the caveats with the regression estimator.   
%\citep{Glynn_2010}. 
