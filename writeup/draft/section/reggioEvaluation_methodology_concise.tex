...
\textbf{[JJH: Then this is a great -- clean -- comparison. Group, we should feature more.][We have included an analysis of age-50. Please see Section~\ref{subsubsection:age50}.]}. 

\textbf{[JJH: Can we evaluate this within cohorts?]} \textbf{[Team: We provide these within-cohort evaluations in the appendix. We have changed the text in this paragraph to make this clearer. Would you prefer to have it in the main paper?] [JJH: Yes, if possible.][We have included these results. Please see Section~REFER for a description of the analysis and Section~\ref{subsubsection:itc} for a description of the results. Overall, the results are weak: there are few significant effects and several are negative such as cognitive skills.]}

\subsection{Within-City Analysis} \label{sec:within-city-analysis}
\subsubsection{OLS Framework to Evaluate Reggio Approach Preschool} \label{subsubsection:OLS}
...
We limit the sample to Reggio and a specific cohort constrained to the comparison groups needed according to the hypotheses above. To test $H_1$, we estimate the following regression equation:
\begin{eqnarray}
Y_{i}^{c,h} & = & \alpha + \beta_{0}R_i^{ITC} + \mathbf{X}_i \bm{\gamma} + \varepsilon_{i}^{Reggio,h}, \\ \nonumber
& \forall & i \in \text{ \{Individuals in Reggio Emilia and cohort $h$ and in group (0,0) or (1,0)\}}
\end{eqnarray}
...
\begin{eqnarray}
Y_{i}^{c,h} & = & \alpha + \beta_{0}R_i^{ITC} + \mathbf{X}_i \bm{\gamma} + \varepsilon_{i}^{Reggio,h}, \\ \nonumber
& \forall & i \in \text{ \{Individuals in Reggio Emilia and cohort $h$ and in group (0,1) or (1,1)\}.}
\end{eqnarray}
...
\subsubsection{Comparison to the Age-50 Cohort} \label{subsubsection:age50}
