...
We adopt multiple approaches to evaluate the effects of the Reggio Approach Preschool intervention. In Section \ref{subsubsection:OLS}, we describe an OLS model that allows us to compare the outcomes of those who attended Municipal Preschool in Reggio to those from Reggio who did not attend Municipal Preschool. In Section \ref{subsubsection:DID}, we present a Difference-in-Differences (DiD) approach that allows us to make cross-city comparisons of outcomes for those who attended  Municipal preschool in each city, while controlling for permanent differences between cities. In Section \ref{subsubsection:psm}, we describe a propensity score matching model to calculate treatment effects. In Section \ref{subsubsection:aipw}, we present an AIPW model that adjusts estimated treatment effects to account for potential selection into treatment.  Lastly, Section \ref{subsection:selection} presents a multinomial logit model to test for the presence of selection into treatment.
...
\end{eqnarray}
where  Equation \ref{eq:specific2} is estimated seperately for the age 30 and 40 cohorts, and $Reggio_i$ is the indicator for individual $i$ having attended preschool in Reggio Emilia. $\beta_3$ is interpreted as the difference that remains between Municipal schools in Reggio and Parma after adjusting for permanent differences between the two cities. In other words, $\beta_3$ is the DiD treatment effect estimator that amounts to (Reggio Municipal - Parma Municipal) - (Reggio None - Parma None), where the first difference captures the unadjusted (for permanent city differences) difference between Municipal preschools in each city, and the second difference captures the permanent differences between the two cities. Analogous interpretations are applied to difference-in-difference comparisons between Reggio Emilia and Padova and comparisons between municipal schools and other school types.


\subsubsection{Propensity Score Matching}  \label{subsubsection:psm}

We estimate propensity score matching model that implements nearest-neighbor matching on an estimated propensity score based on a set of observed baseline characteristics $\boldsymbol{X}_i$. This approach is based on the assumption that adjusting for the propensity score is enough to eliminate all confounding. Formally writing, the average treatment effect is,
...
Although the baseline OLS and difference-in-difference models attempt to account for the selection into the Reggio Approach, regression estimator for ATE may be poorly estimated when the observed values of baseline variables are not similar for the treatment and control groups. We estimate an Augmented Inverse Propensity Weighted (AIPW) estimator in order to account for the caveats with the regression estimator.   
%\citep{Glynn_2010}. 
