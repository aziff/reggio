...
\subsection{Accounting for Selection into Treatment}

\subsubsection{Investigating Differences in Selection across Cities: A Multinomial Logit Approach}
We seperately estimate multinomial logit models for each city using preschool choice as the dependent variable, and a vector of baseline characteristics as independent variables. We then make cross-city comparisons of the estimated marginal effects for each baseline characteristic to investigate whether a common set of baseline variables determine selection into Municipal preschool in each city. This analysis provides sample-based insights into parents' perception of the quality of Municipal schools relative to other alternatives available within the same city. Substantial differences in selection across cities would suggest that parents in one city perceive Municipal schools differently than parents in another city, relative to the alternatives available within their respective cities. 

To begin, we define the choices avaiable to parents in each city as follows:
\begin{equation}
S_i = \quad
\begin{cases}
0 \quad \text{if \quad (\textit{i} Attended Municipal preschool)} \\
1 \quad  \text{if \quad (\textit{i} Attended an Alternative preschool)} \\
2 \quad \text{if \quad (\textit{i} Did not attend any preschool)}  \\
\end{cases}
\end{equation}

\noindent We then estimate the following multinomial logit specification seperately for each city:

\begin{equation} \label{eq:log-odds-ratio}
\nu_r = \text{log} \left(\frac{Pr(S_i=r)}{Pr(S_i=0)}\right) = \beta_0^r + \bm{\beta^r X} + \epsilon_i^r \quad \forall \quad r \in 1,2
\end{equation}

where $\bm{X}$ is a vector of baseline characteristics including child's gender, number of child's siblings, religiosity of caregiver, mothers education level, and indicators for whether parents were born in province.

The predicted log-odds ratio, $\hat{\nu}$, from (\ref{eq:log-odds-ratio}) can be used to contruct the estimated probability of attending each school type $r$ as follows:
...
D_i = \quad
\begin{cases}
1 \quad if \quad &  R_i = 1\text{\quad 		(Attended Reggio Appraoch)} \\
0 \quad if \quad &  R_i \neq 0 \text{\quad 	(From Reggio but did not attend Reggio Approach)} \\
\end{cases}
\end{equation}
~\\
...
T_i = \quad
\begin{cases}
0 \quad \text{if \quad (\textit{i} From Reggio and attended Reggio Approach)} \\
1 \quad  \text{if \quad (\textit{i} From Reggio and attended no preschool)} \\
2 \quad \text{if \quad (\textit{i} From Reggio and attended other preschool)}  \\
\end{cases}
\end{equation}
~\\ ~\\
...
\widehat{IPW}_j=\widehat{E[Y(0)-Y(j)]}_{IPW} = \frac{1}{n} \sum_{i=1}^{n} \left \{\frac{(\mathbb{1}_{T_i=0}) Y_i}{\hat{\Pi_0}(\boldsymbol{X}_i)} - \frac{(\mathbb{1}_{T_i=j})Y_i}{\hat{\Pi_j}(\boldsymbol{X}_i)}\right \}
\end{equation}
~\\
...
\widehat{AIPW_{j}} = E[\widehat{Y(0)-Y(j)]}_{AIPW} =& \frac{1}{n} \sum_{i=1}^{n}\bigg \{ \bigg[ \overbrace{ \frac{(\mathbb{1}_{T_i=0}) Y_i}{\hat{\Pi_0}(\boldsymbol{X}_i)} - \frac{(\mathbb{1}_{T_i=j})Y_i}{\hat{\Pi_j}(\boldsymbol{X}_i)}}^{IPTW}  \bigg]- \frac{(\mathbb{1}_{T_i=0}) \hat{\Pi}_j(\boldsymbol{X}_i) - (\mathbb{1}_{T_i=j}) \hat{\Pi}_0(\boldsymbol{X}_i)}{\hat{\Pi}_0(\boldsymbol{X}_i) \, \hat{\Pi}_j(\boldsymbol{X}_i)} \nonumber \\[10pt]
& \bigg[ \hat{\Pi}_j(\boldsymbol{X}_i) \cdot E[\hat{Y_i}|T_i=0,\boldsymbol{X}_i] +\hat{\Pi}_0(\boldsymbol{X}_i) \cdot E[\hat{Y_i}|T_i=j,\boldsymbol{X}_i] \bigg] \bigg \}
\end{align}