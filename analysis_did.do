...
local adult_outcome_S				MrgrTaste Friends MigrFriend					
	

** Cohorts, cities, and schools									
...
local MrgrTaste_short			MTS
local Friends_short				FRN
local MigrFriend_short			HMF
...
foreach g in p m f  {
	foreach o in E W L H N {
		...
		if `bic_switch' == 1 {
			local controls `bic_baseline_vars'
			local folder 	bic
		}
		...
		foreach city in Reggio Parma Padova {	
			foreach var in `adult_outcome_`o'' {
				* Estimate
				...
				if r(N) > 0 {
					reg `var' `X' `controls' if `city' == 1 ``g'_con'
					mat rslt = r(table)
		...
		foreach cohort in Adult30 Adult40 Adult50 {
			foreach var in `adult_outcome_`o'' {
				* Estimate
				...
				if r(N) > 0 {
					reg `var' `X' `controls' if Cohort_`cohort' == 1 ``g'_con'
					mat rslt = r(table)
