	...
	foreach city in `cities' {
		...
		foreach out_type in E W L H N S {
			...
			foreach v in `adult_outcome_`out_type'' {
				...
				* Conditional mean
				reg `v' ib1.maternaType `adult_baseline_vars_`out_type''
				cap matrix drop cond
				matrix cond = r(table)
				local constant = cond[1,6]
				local cond_r_squared = e(r2)
				...
				foreach l in cond_r_squared cond_meanMunicipal cond_meanState cond_meanReligious cond_meanPrivate cond_meanNone {
					local `l' : di %9.2f ``l''
				}
				foreach tmp_type in Municipal State Religious Private None {
					if `p`tmp_type'' <= 0.1 {
						local cond_mean`tmp_type' \textbf{`cond_mean`tmp_type''}
					}
				}
				...
				foreach s in `schools' {
					...
					* Unconditional mean
					...
					// save to row 
					if `school_i' == 1 {
						local row `row' 	`vl' & `mean_save'  & `cond_mean`school'' 
					}
					else {
						local row `row' 	& `mean_save' & `cond_mean`school''
					}
