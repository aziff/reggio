...
local Cognitive			${child_outcome_C} ${adol_outcome_C}
local NonCognitive		${child_outcome_N} ${adol_outcome_N} ${adult_outcome_N}
local Social 			${child_outcome_S} ${adol_outcome_S} ${adult_outcome_S}
local Health 			${child_outcome_H} ${adol_outcome_H} ${adult_outcome_H}
local Employment 		${adult_outcome_W}
local Education			votoMaturita votoUni highschoolGrad MaxEdu_Uni MaxEdu_Grad
local LivingStatus		${adult_outcome_L}

foreach m in Cognitive NonCognitive Social Health Employment Education LivingStatus{
	local `m': list uniq `m'
}
...
foreach category in Cognitive NonCognitive Social Health Employment Education LivingStatus{
	file write dstat "\textbf{`category'} \\[.6em]" _n
