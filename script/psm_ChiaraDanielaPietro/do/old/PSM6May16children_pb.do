...
*** global
* cambiato rispetto ad adolescents
global outcomes = "childSDQ_score faceMe childHealthPerc"
...
if 1==1{ // Summary statistics
	local outChild $outcomes

	*tables of outcomes
	local options se sdbracket vert nototal nptest //sd mtprob mtest bdec(3) ci cibrace nptest 
	local group cityXasilo
	tabformprova `outChild'  using child_OUTCOMEasilo if Cohort==1, by(`group') `options'
	local group cityXmaterna
	tabformprova `outChild'  using child_OUTCOMEmaterna if Cohort==1, by(`group') `options'

	*table of outcomes, controls, and IV
	local group asiloG_1_2
	tabformprova $outcomes $controls $IVnido using child_CONTROL1asilo if Cohort==1, by(`group') `options'
	local group asiloG3
	tabformprova $outcomes $controls $IVnido using child_CONTROL2asilo if Cohort==1, by(`group') `options'
	local group maternaG_1_2
	tabformprova $outcomes $controls $IVmaterna using child_CONTROL1materna if Cohort==1, by(`group') `options'
	local group maternaG3
	tabformprova $outcomes $controls $IVmaterna using child_CONTROL2materna if Cohort==1, by(`group') `options'
}
...
* cambiato rispetto ad adolescents
biprobit (supply = score50 Male Age momMaxEdu_Uni_F momMaxEdu_Uni_Miss HighInc_F HighInc_Miss houseOwn_F houseOwn_Miss) (demand = cgAsilo_F distAsiloMunicipal1 distanza2 distanza3 momBornProvince_F dadBornProvince_F momBornProvince_Miss dadBornProvince_Miss Male Age momMaxEdu_Uni_F momMaxEdu_Uni_Miss HighInc_F HighInc_Miss houseOwn_F houseOwn_Miss) if(Reggio == 1), partial difficult
...
gen weight0 = 1
* cambiato rispetto ad adolescents
reshape long childSDQ_score faceMe childHealthPerc weight intnr nido Parma Padova, i(id) j(ReggioAsilo)
count
tab ReggioAsilo
tab weight
foreach j in $outcomes {
...
* cambiato rispetto ad adolescents
biprobit (supply = score50 Male Age momMaxEdu_Uni_F momMaxEdu_Uni_Miss houseOwn_F houseOwn_Miss) (demand = cgAsilo_F distMaternaMunicipal1 momBornProvince_F dadBornProvince_F momBornProvince_Miss dadBornProvince_Miss Male Age houseOwn_F houseOwn_Miss) if(Reggio == 1), partial difficult
outreg2 using "${klmReggio}/Analysis/chiara&daniela/resultsPSM2.tex", append tex(frag) $outregoptions 
...
gen weight1 = 1
gen weight0 = 1
* cambiato rispetto ad adolescents QUI
reshape long childSDQ_score faceMe childHealthPerc weight intnr nido Parma Padova, i(id) j(ReggioMaterna)
count
tab ReggioMaterna
tab weight
foreach j in $outcomes {
