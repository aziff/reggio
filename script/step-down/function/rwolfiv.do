...
foreach var of varlist `varlist' {
    local ++j
	cap qui `method' `var' `controls' (`indepvar' = $IVinstruments) `if' `in' [`weight' `exp'], robust first
	if _rc!=0 {
    ...
    `method' `var' `controls' (`indepvar' = $IVinstruments) `if' `in' [`weight' `exp'], robust first;
    #delimit cr
    preserve
