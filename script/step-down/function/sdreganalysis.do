...
capture program drop sdreganalysis
capture program define sdreganalysis
		...
		foreach comp in ${reglist} {
			...
			if r(N) > 0 {
				...
				* Add to the matitems and matnames locals
				if `switch' == 1 {
					local matitems `matitems' `itt_`comp'', `itt_`comp'_se', `itt_`comp'_p', `itt_sd_`var'_`comp'', `itt_`comp'_N' 
				}
				if `switch' == 0 {
					local matitems `matitems', `itt_`comp'', `itt_`comp'_se', `itt_`comp'_p', `itt_sd_`var'_`comp'', `itt_`comp'_N'  
				}
				
				local matnames `matnames' itt_`comp' itt_`comp'_se itt_`comp'_p itt_`comp'_sdp itt_`comp'_N
