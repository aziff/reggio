/* ---------------------------------------------------------------------------- *
* This program estimates DID where each diff is estimated using matching.
* One of the matched-diff estimates differences between the "treated" groups, and
* the other estimates differences between the "control" groups. Both diffs are taken
* across cities:(Reggio Treated-Comparison Treated),(Reggio Control-Comparison Control).
* Each diff matches individuals from the comparison city to individuals in Reggio. 
...
capture program drop matchedDID
capture program define matchedDID
