...
include "${here}/function/kernelanalysis"
...
foreach stype in Other /*Stat Reli*/ {
	...
	* Set necessary global variables
	...
	global controlsKM			${bic_child_baseline_vars}
	...
	foreach type in  M /*CN S H B*/ {
		...
		* ----------------------- *
		* Open necessary files
		file open kern_`type'_`stype' using "${git_reggio}/output/multiple-methods/combinedanalysis/csv/kern_child_`type'_`stype'.csv", write replace

		* Run Multiple Analysis
		di "Estimating `type' for Children: PSM Analysis"
		kernelanalysis, stype("`stype'") type("`type'") psmlist("${kernellist}") cohort("child")
