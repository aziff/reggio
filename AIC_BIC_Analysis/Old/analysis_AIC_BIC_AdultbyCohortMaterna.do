...
local outcomesAdult		all_houseOwn binSatisFamily binSatisHealth binSatisIncome binSatisWork
						...
						Reddito_4 Reddito_5 Reddito_6 SES_self SickDays Smoke sport votoMaturita 
						votoUni WageMonth;
...
/* The following vars are exluded from list of outcomes because they either had 0 obs for adult cohort,
   or there was no variance in the variable for adult cohort i.e., it was a dummy with all values = 0:
						...
						Reddito_7, i_RiskDUI i_RiskFight RiskSuspended SDQ_score */
...
file open file1 using "./Output/AIC_BIC_AdultbyCohortMaterna.csv", write replace
file write file1 "Cohort,Tuple,Controls,Fixed Covariate,Dependent Variable,AIC,BIC,N" _n // 1st row = variable names

local cohort_val = 4 // Adult30 = 4, Adult40 = 5, Adult50 = 6
foreach cohort in "Adult 30" "Adult 40" "Adult50"{
	foreach outcome of local outcomesAdult{
		forvalues i = 1/`max_tuple'{
			reg	`outcome' Reggio `tuple_`i'' if Cohort == `cohort_val' & materna==1 // restricting cohort by age of adult, and making sure respondent attended preschool
			quietly estat ic //command creates matrix r(S) where 1st, 5th and 6th elements are N, AIC and BIC respectively 
			matrix aa =  r(S)
			local num =  aa[1,1]
			local aic =  aa[1,5]
			local bic =  aa[1,6]
			file write file1 "`cohort_val', tuple_`i',`tuple_`i'',Reggio,`outcome',`aic',`bic',`num'" _n
		}
	}
...
import delimited "./Output/AIC_BIC_AdultbyCohortMaterna.csv", varnames(1) delimiters(",") clear
...
file open file2 using "./Output/Average_AIC_BIC_AdultbyCohortMaterna.csv", write replace
