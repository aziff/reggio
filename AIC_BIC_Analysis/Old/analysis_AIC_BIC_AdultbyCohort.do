...
file open file1 using "./Output/AIC_BIC_AdultbyCohort.csv", write replace
...
foreach cohort in "Adult 30" "Adult 40" "Adult50"{
	foreach outcome of local outcomesAdult{
		forvalues i = 1/`max_tuple'{
			reg	`outcome' Reggio `tuple_`i'' if Cohort == `cohort_val' // restricting cohort by age of adult
...
import delimited "./Output/AIC_BIC_AdultbyCohort.csv", varnames(1) delimiters(",") clear
...
file open file2 using "./Output/Average_AIC_BIC_AdultbyCohort.csv", write replace
