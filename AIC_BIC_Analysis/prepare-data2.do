...
* Current version:  06/26/16

* Sid's Note: This modifies prepare-data.do by using all cohort values, and not
* just Child, Adol and Adult.
* ---------------------------------------------------------------------------- *
...
** Sid edit 6/26/16 - we create a new Cohort_new var to account for all cohorts
generate Cohort_new1 = 0
replace Cohort_new1 = 1 if Cohort == 1 // children
replace Cohort_new1 = 2 if Cohort == 2 // migrants
replace Cohort_new1 = 3 if Cohort == 3 // adolescents
replace Cohort_new1 = 4 if Cohort == 4 // adults30
replace Cohort_new1 = 5 if Cohort == 5 // adults40
replace Cohort_new1 = 6 if Cohort == 6 // adults50
...
** Sid edit 6/26/16 - I have changed `cohorts' to `cohorts_new' below.
...
* ----------------------------------------------------------------------------- *
* Create more baseline variables
** Catholic
...
** Religiousness
sum cgFaith, detail
gen cgFaith_dummy = (cgFaith >= r(p50))
label var cgFaith_dummy "Caregiver is more religious than or as religious as the average"

** Interaction of Catholic and Religiouness
gen int_cgCatFaith = cgCatholic * cgFaith
label var int_cgCatFaith "Caregiver is Catholic and relatively faithful"

** Sibling dummies
gen numSibling_0 = (numSiblings == 0)
label var numSibling_0 "Number of sibling: 0"
gen numSibling_1 = (numSiblings == 1)
label var numSibling_1 "Number of sibling: 1"
gen numSibling_2 = (numSiblings == 2)
label var numSibling_2 "Number of sibling: 2"
gen numSibling_more = (numSiblings >= 3)
label var numSibling_more "Number of sibling: more than 3"
